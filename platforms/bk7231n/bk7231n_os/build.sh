...
# ---- Standard debug helpers ----
dump_config_and_bls() {
  echo "===== DEBUG: config.json (first 120 lines if present) ====="
  sed -n '1,120p' config.json 2>/dev/null || echo "config.json not yet created"
  echo "===== DEBUG: candidate bootloader files ====="
  ls -l bk7231n_bootloader_enc.bin bk7231n_bootloader_zero_keys.bin 2>/dev/null || true
  if command -v sha256sum >/dev/null 2>&1; then
    [ -f bk7231n_bootloader_enc.bin ] && sha256sum bk7231n_bootloader_enc.bin || true
    [ -f bk7231n_bootloader_zero_keys.bin ] && sha256sum bk7231n_bootloader_zero_keys.bin || true
  else
    echo "sha256sum not available"
  fi
}

hash_first_64k_of_qio() {
  if [ -f "${APP_BIN_NAME}_QIO_${APP_VERSION}.bin" ]; then
    echo "===== DEBUG: first 64KB of QIO hash ====="
    head -c 65536 "${APP_BIN_NAME}_QIO_${APP_VERSION}.bin" > bl_from_qio.bin
    if command -v sha256sum >/dev/null 2>&1; then
      sha256sum bl_from_qio.bin
    else
      echo "sha256sum not available"
    fi
  fi
...
if [ "$BUILD_MODE" = "zerokeys" ] && [ "$PLAT_UPPER" = "BK7231M" ]; then
