...
if [ "$BUILD_MODE" = "zerokeys" ]; then
	...
	python mpytools.py bk7231n_bootloader.bin ${APP_BIN_NAME}_${APP_VERSION}_enc.bin
else
...
if [ -z $CI_PACKAGE_PATH ]; then
   ...
   cp ././${APP_PATH}/$APP_BIN_NAME/output/$APP_VERSION/OpenBK7231N_UASCENT_QIO_${APP_VERSION}.bin ${CI_PACKAGE_PATH}/$FW_NAME"_UASCENT_QIO_"$APP_VERSION.bin || true
   cp ././${APP_PATH}/$APP_BIN_NAME/output/$APP_VERSION/OpenBK7231N_UASCENT_UA_${APP_VERSION}.bin ${CI_PACKAGE_PATH}/$FW_NAME"_UASCENT_UA_"$APP_VERSION.bin  || true
fi
